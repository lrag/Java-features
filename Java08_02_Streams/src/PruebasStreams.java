...
public class PruebasStreams {
	...
	public void movidas() {
		...
		ServicioMensajes sm = new ServicioMensajes();
		//IMPERATIVO
		/*
		...
		*/
		...
		facturas
			...
			.forEach( cli -> sm.enviarCorreoE(cli.getNombre(), "OLA KE TAL, GRAZIAS POR SU KONPRA") );				
		...
		*/
		...
		//
		Map<Boolean, List<Integer>> resultado = numeros.stream().collect(Collectors.partitioningBy( num -> num % 2 == 0));
		System.out.println(resultado.get(true));
		System.out.println(resultado.get(false));
		...
		*/
		...
		Arrays.asList("a1", "a2", "b1", "c1", "c2", "D1", "D2", "E1", "E2", "E3", "E4")
			.parallelStream()
			.filter(s -> {
				try {
					Thread.sleep(250);
